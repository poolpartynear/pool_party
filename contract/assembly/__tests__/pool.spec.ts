import {
  deposit_and_stake_callback, deposit_and_stake, unstake,
  get_accum_weights, init
} from '..';
...
describe("Initializing", () => {
  it("cannot be initialized twice", () => {
    ...
    expect(() => { init("pool", "guardian", "dao", 0) }).not.toThrow()
    expect(() => { init("e", "f", "g", 0) }).toThrow()
...
describe("User Handling", () => {
  it("correctly updates total_stake, to_unstake variables", () => {
    set_context(Context.contractName, u128.Zero)
    init('external_pool', 'theguardian', 'dao', 0)  // init the contract
